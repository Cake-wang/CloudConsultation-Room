...
/**
 * 前置条件：
 * 需要Retrofit，需要 OKhttp， 需要 FastLib
 * 用于快速搭建 retrofit
 * 用于自定义过滤数据转换过程中，出现的null对象，并添加默认值，【这些默认值都是警示值，告诉系统，数值异常，但是不会让系统崩溃】
 *
 *
 * @author louisluo
...
class MFastMultiUrl {
    ...
    private static volatile MFastMultiUrl sInstance;
    /**
     * retrofit 设置的BaseUrl
     */
    private String mBaseUrl;

    public static MFastMultiUrl getInstance() {
        if (sInstance == null) {
            synchronized (MFastMultiUrl.class) {
                if (sInstance == null) {
                    sInstance = new MFastMultiUrl();
                }
            }
        }
        return sInstance;
     ...
     * @return
     */
    public MFastMultiUrl with(OkHttpClient.Builder builder) {
    ...
    public Request processRequest(Request request) {
        ...
        if (null != httpUrl) {
            ...
            LoggerManager.i(MFastMultiUrl.TAG, "Header 模式重定向Url:Base Url is { " + mBaseUrl + " }" + ";New Url is { " + newUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(newUrl)
                    .build();
        }
        ...
        if (null != httpUrl) {
            LoggerManager.i(MFastMultiUrl.TAG, "Method 模式重定向Url:Base Url is { " + mBaseUrl + " }" + ";New Url is { " + httpUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpUrl)
                    .build();
        }
        ...
        if (httpUrlBase != null && !httpUrlBase.toString().equals(mBaseUrl)) {
            ...
            LoggerManager.i(MFastMultiUrl.TAG, "重定向Url:Base Url is { " + httpUrlBase.toString() + " }" + ";New Url is { " + httpNew + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpNew)
                    .build();
        }
     ...
     * @param enable
     */
    public MFastMultiUrl setIntercept(boolean enable) {
     ...
     * @return
     */
    public MFastMultiUrl setHeaderPriorityEnable(boolean enable) {
     ...
     * @param url
     */
    public MFastMultiUrl setGlobalBaseUrl(String url) {
    ...
    public MFastMultiUrl putHeaderBaseUrl(Map<String, String> map) {
     ...
     * @param urlValue
     */
    public MFastMultiUrl putHeaderBaseUrl(String urlKey, String urlValue) {
     ...
     * @param urlKey
     */
    public MFastMultiUrl removeHeaderBaseUrl(String urlKey) {
        synchronized (mHeaderBaseUrlMap) {
     ...
     * 清除所有BaseUrl
     */
    public MFastMultiUrl clearAllHeaderBaseUrl() {
     ...
     * @param parser
     */
    public MFastMultiUrl setUrlParser(FastUrlParser parser) {
        this.mUrlParser = parser;
        return sInstance;
    }

    public MFastMultiUrl putBaseUrl(Map<String, String> map) {
        if (map != null && map.size() > 0) {
            for (Map.Entry<String, String> entry : map.entrySet()) {
     ...
     * @return
     */
    public MFastMultiUrl putBaseUrl(String urlKey, String urlValue) {
        synchronized (mBaseUrlMap) {
    ...
    public MFastMultiUrl removeBaseUrl(String urlKey) {
        synchronized (mBaseUrlMap) {
     ...
     * 清除所有BaseUrl
     */
    public MFastMultiUrl clearAllBaseUrl() {
        mBaseUrlMap.clear();
        return sInstance;
    }
