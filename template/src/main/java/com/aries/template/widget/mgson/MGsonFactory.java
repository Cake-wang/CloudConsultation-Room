 ...
 */
public final class MGsonFactory extends Converter.Factory {
  /**
   * Create an instance using a default {@link Gson} instance for conversion. Encoding to JSON and
   * decoding from JSON (when no charset is specified by a header) will use UTF-8.
   */
  public static MGsonFactory create() {
//    return create(new Gson());
    return create(new GsonBuilder()
            .registerTypeAdapterFactory(new FullTypeAdapterFactory())
            .create());
  }
  ...
  public static MGsonFactory create(Gson gson) {
    if (gson == null) throw new NullPointerException("gson == null");
    return new MGsonFactory(gson);
  }

  private final Gson gson;

  private MGsonFactory(Gson gson) {
    this.gson = gson;
  }
